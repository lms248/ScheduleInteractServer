 ...
 * 获取好友日程列表
 * */
@WebServlet(description = "获取好友日程列表", urlPatterns = { "/RCGLServer/GetFriendScheduleListToClient" })
public class GetFriendScheduleListToClient extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public GetFriendScheduleListToClient() {
        super();
    }
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("*************GetFriendScheduleListToClient*****************");
		...
		if(userid.equals("0")){
			...
			if(scheduleList.size()==0){
				System.out.println("该好友没有公开日程");
				out.write("0");
				return;
			}
			for(int i=0;i<scheduleList.size();i++){
				if(scheduleList.get(i).getOpen()==0){
					continue;
				}
