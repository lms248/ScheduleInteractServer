...
$(document).ready(function(){
	...
	//$("select").selectpicker({style: 'btn-primary', menuStyle: ''});
	...
	$("#create-user").on("click",function(){
		...
		$.myconfirm("确定创建: ["+username+"] ?",function(){
			...
			$.post("/rcgl/user/create.do",requestData,function(data, textStatus, jqXHR){
				var json=JSON.parse(data);
				$.myalert(json.data);
				if(json.code==0){
					$(".main").load("/rcgl/view/user-index.jsp");
				}
			},"json");
		});
	});
	...
	$("#search-user-btn").on("click",function(){
		...
		$(".main").load("/rcgl/view/user-index.jsp?searchKey="+key);
	});
	
	$("a[name='lookuser']").on("click",function(){
		var userid=$(this).attr("userid");
		var requestData={userid:userid};
		$.post("/rcgl/user/look.do",requestData,function(data, textStatus, jqXHR){
			/* var json=JSON.parse(data); */
			var json=data; 
			if(json.code==0){
	...
	$("a[name='deluser']").on("click",function(){
		...
		$.myconfirm("确定删除 ?",function(){
			...
			$.post("/rcgl/user/delete.do",requestData,function(data, textStatus, jqXHR){
				var json=JSON.parse(data);
				$.myalert(json.data);
				if(json.code==0){
					$(".main").load("/rcgl/view/user-index.jsp");
	...
	$("a[name='update-user']").on("click",function(){
		var userid=$(this).attr("userid");
		var requestData={userid:userid};
		$.post("/rcgl/user/pre-update.do",requestData,function(data, textStatus, jqXHR){
			var json=JSON.parse(data);
			if(json.code==0){
	...
	$("#udpate-user-submit").on("click",function(){
		...
		$.myconfirm("确定修改 ?",function(){
			...
			$.post("/rcgl/user/update.do",requestData,function(data, textStatus, jqXHR){
				var json=JSON.parse(data);
				$.myalert(json.data);
				if(json.code==0){
